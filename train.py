...
def run(rank, n_gpus, hps):
    global global_step, eval_loader
    if rank == 0:
        logger.info(hps)
    ...
    if rank == 0:
        ...
        eval_loader = DataLoader(eval_dataset, num_workers=1, shuffle=False, batch_size=1, pin_memory=False,
    ...
    try:
        ...
        logger.error("load old checkpoint failed...")
        epoch_str = 1
        global_step = 0
    ...
    logger.info("The latest epoch was {}, global step was {}", epoch_str, global_step)
    ...
    for epoch in range(epoch_str, hps.train.epochs + 1):
        ...
        if epoch <= warmup_epoch:
            lr = hps.train.learning_rate / warmup_epoch * epoch
        ...
        if rank == 0:
            if global_step % hps.train.log_interval == 0:
                ...
                logger.info(f"Losses: {[x.item() for x in losses]}, "
                            f"Step: {global_step}, "
                            f"Learning rate: {lr}, "
                            f"Reference loss: {reference_loss}")
...
def evaluate(hps, generator, eval_loader, writer_eval):
    ...
    utils.summarize(writer=writer_eval, global_step=global_step, images=image_dict, audios=audio_dict,
