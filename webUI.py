...
def vc_fn2(sid, input_audio, vc_transform, auto_f0, cluster_ratio, slice_db, noise_scale, pad_seconds, cl_num, lg_num,
    ...
    save_path2 = f"{text2tts[0:10]}_44k.wav"
    wavfile.write(save_path2, sr2, (wav2 * np.iinfo(np.int16).max).astype(np.int16))
            ...
            with gr.Tabs():
                with gr.TabItem("静态声线融合"):
                    ...
                    mix_model_upload_button = gr.UploadButton("选择/追加需要混合模型文件", file_count="multiple",
                                                              variant="primary")
                    mix_model_output1 = gr.Textbox(label="混合比例调整，单位/%", interactive=True)
